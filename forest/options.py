...
def split_net_names(s):
    """Splits the net names by comma and returns a list of names."""
    return [str(item) for item in s.split(',')]

def split_save_targets(s):
    """Splits the save targets by comma and returns a list of targets."""
    return [str(item) for item in s.split(',')]

def split_load_sources(s):
    """Splits the load sources by comma and returns a list of sources."""
    return [str(item) for item in s.split(',')]
def options():
    ...
    ###########################################################################
    # Central:
    parser.add_argument('--net', default='resnet18', type=split_net_names)
    ...
    parser.add_argument('--budget', default=0.5, type=float, help='Fraction of training data that is poisoned')
    parser.add_argument('--sigma', default=1e-3, type=float, help='Correction constant')
    parser.add_argument('--lb', default=10, type=float, help='Sponge penalty')
    ...
    parser.add_argument('--retrain_max_epoch', default=100, type=int,
                        help='Final_Training only up to this epoch before poisoning.')
    parser.add_argument('--val_max_epoch', default=100, type=int,
                        help='Final_Training only up to this epoch before poisoning.')
# python -u sponger.py --net="resnet18" --dataset=GTSRB --epochs=100 --max_epoch=100 --scenario="from-scratch" --noaugment --batch_size=512 --optimization="sponge_exponential"  --sources=100  --budget=0.05 --sigma=1e-01
    ...
    parser.add_argument('--ablation', default=0.01, type=float,
    ...
    parser.add_argument('--save', default='net,sponge-net',
                        type= split_save_targets,
                        help='Export clean and sponge net.')
    parser.add_argument('--load', default='net', type=split_load_sources)
