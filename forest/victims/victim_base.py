...
class _VictimBase:
    ...
    def poison_train(self, kettle, max_epoch=None):
        """poisoning (pre)-training of the chosen model"""
        print('\nStarting poisoning training ...\n')

        if self.args.scenario == "from-scratch":
            stats_clean = self._iterate(kettle, poison_delta='Poisoning', max_epoch=max_epoch,
    ...
    def _poisoning_step(self, kettle, train_loader, valid_loader, x_train_poisoned, y_train_poisoned, class_descr, test_inputs, test_labels, opt, schedulerr, poison_delta, epoch, stats, model, defs, optimizer, scheduler, pretraining_phase=False):
        # run_poisoning_step(kettle, poison_delta, epoch, stats, model, defs, optimizer, scheduler,
        #          loss_fn=self.loss_fn, pretraining_phase=pretraining_phase)
        model.frozen = False
        run_poisoning_step(kettle, train_loader, valid_loader, x_train_poisoned, y_train_poisoned, class_descr, test_inputs, test_labels, opt, schedulerr, poison_delta, epoch, stats, model, defs, optimizer, scheduler,
                        loss_fn=self.loss_fn, pretraining_phase=pretraining_phase)
    ...
    def poisoning_attack_setup(self, kettle, poison_delta, epoch, stats, model, defs, optimizer, scheduler, pretraining_phase=False):
        # attack_type = "FeatureCollision"
        attack_type = "MatchingGradient"
        train_loader, valid_loader, inputs, labels, test_inputs, test_labels, opt, scheduler = get_input_output(kettle, model, optimizer, 
                                                                                                                pretraining_phase=False)
        x_train_poisoned, y_train_poisoned, x_test_poisoned, y_test_poisoned, class_descr = final_attack(attack_type ,  kettle, model, inputs, labels, test_inputs, test_labels, optimizer)
        return train_loader, valid_loader, x_train_poisoned, y_train_poisoned, class_descr, x_test_poisoned, y_test_poisoned, opt, scheduler
