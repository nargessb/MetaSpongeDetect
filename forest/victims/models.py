...
class TransformNet(torch.nn.Module):
    def __init__(self, dnn, dnn_name, dataset_name):
        ...
        if dataset_name.upper() == "TINYIMAGENET":
    ...
    def forward(self, x):
        if x.size(1) == 1:
            x = x.repeat(1, 3, 1, 1)  # Repeat the channel dimension to make it 3 channels

        # Pass the data through the rest of the model
        z = self.dnn(x)
        return z
...
def change_last_layer(model, model_name, dataset_name):
    ...
    if 'resnet' in model_name:
        print("FINAL OUTPUT LAYER =", model.dnn.classifier)
...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name:
        if pretrained:
            raise ValueError('Pretrained models are not available for small datasets with VGG16.')
        else:
            in_channels = 3  # Since we've converted MNIST images to 3 channels
            num_classes = 10 if dataset_name in ['CIFAR10', 'MNIST'] else 100
            if model_name.lower() == 'vgg16':
                model = VGG('VGG16_CIFAR', in_channels=in_channels, num_classes=num_classes)
    ...
    elif dataset_name.upper() == 'TINYIMAGENET':
        in_channels = 3
        num_classes = 200
        print(">>> Building TinyImageNet model with 200 classes")
        ...
        if pretrained:
            ...
            if model_name.lower() == 'vgg16':
                model = VGG('VGG16_TinyImageNet', in_channels=in_channels, num_classes=num_classes)
            elif 'resnet' in model_name.lower():
                model = resnet_picker(model_name, dataset_name)
                print(">>> Changing last layer to 200 classes for TinyImageNet")
        ...
        num_classes = 43
        if pretrained:
            raise ValueError('Pretrained models are not available for GTSRB with VGG16.')
        else:
            if model_name.lower() == 'vgg16':
                model = VGG('VGG16_CIFAR', in_channels=in_channels, num_classes=num_classes)
            elif 'resnet' in model_name.lower():
