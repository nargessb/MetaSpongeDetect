...
class StatsRecorder:
    def __init__(self, bitwidth=32, n_gpus=10):
        ...
        if bitwidth not in bitwidth_to_minvalue:
            raise ValueError("Passed bitwidth is not supported")
        self.min_value = bitwidth_to_minvalue[bitwidth]
        self.nonzero_func = self.count_nonzero_based_on_min_value  # Correctly refer to the class method

    def count_nonzero_based_on_min_value(self, x):
        """Counts the non-zero elements based on the minimum value threshold."""
        return float((x.abs() > self.min_value).nonzero().size(0))
...
def record_stats(stats):
    ...
    * Number of non-zeros
    ...
    def wrapper_hook_fn(module, in_data, out_data):
        return hook_fn(stats.nonzero_func, module, in_data, out_data)

    return wrapper_hook_fn  
...
def check_sourceset_consumption(model, kettle, stats):
    ...
    model.eval()
    if kettle.args.batch_size is None:
        kettle.args.batch_size = 512
    print("Batch size before comparison:", kettle.args.batch_size)
